mutable struct Variable{DT}
    value::DT
    parents::Vector{Tuple{Variable{DT}, DT}}
    grad::DT

    function Variable(value::DT) where DT
        new{DT}(value, Tuple{Variable{DT}, DT}[], zero(DT))
    end

end

function Base.:+(a::Variable{DT}, b::Variable{DT}) where DT
    out = Variable(a.value + b.value)
    push!(out.parents, (a, one(DT)))
    push!(out.parents, (b, one(DT)))
    return out
end

function Base.:+(a::Variable{DT}, b::Number) where DT
    out = Variable(DT(b) + a.value)
    push!(out.parents, (a, one(DT)))
    return out
end

function Base.:+(a::Number, b::Variable{DT}) where DT
    return b+a
end

function Base.:*(a::Variable{DT}, b::Variable{DT}) where DT
    out = Variable(a.value * b.value)
    push!(out.parents, (a, b.value))
    push!(out.parents, (b, a.value))
    return out
end

function Base.:*(a::Variable{DT}, b::Number) where DT
    out = Variable(a.value * DT(b))
    push!(out.parents, (a, DT(b)))
    return out
end

function Base.:*(a::Number, b::Variable{DT}) where DT
    return b * a
end

function Base.:-(a::Variable{DT}, b::Variable{DT}) where DT
    out = Variable(a.value - b.value)
    push!(out.parents, (a, one(DT)))
    push!(out.parents, (b, -one(DT)))
    return out
end

function Base.:-(a::Variable{DT}, b::Number) where DT
    out = Variable(a.value - DT(b))
    push!(out.parents, (a, one(DT)))
    return out
end

function Base.:-(a::Number, b::Variable{DT}) where DT
    out = Variable(DT(a) - b.value)
    push!(out.parents, (b, -one(DT)))
    return out
end

function Base.:-(a::Variable{DT}) where DT
    out = Variable(-a.value)
    push!(out.parents, (a, -one(DT)))
    return out
end

function Base.:/(a::Variable{DT}, b::Variable{DT}) where DT
    out = Variable(a.value / b.value)
    push!(out.parents, (a, one(DT) / b.value))
    push!(out.parents, (b, -a.value / (b.value^2)))
    return out
end

function Base.:/(a::Variable{DT}, b::Number) where DT
    out = Variable(a.value / DT(b))
    push!(out.parents, (a, one(DT) / DT(b)))
    return out
end

function Base.:/(a::Number, b::Variable{DT}) where DT
    out = Variable(DT(a) / b.value)
    push!(out.parents, (b, -DT(a) / (b.value^2)))
    return out
end

function Base.:^(a::Variable{DT}, b::Number) where DT
    out = Variable(a.value^b)
    push!(out.parents, (a, DT(b) * a.value^(b - 1)))
    return out
end

function Base.exp(a::Variable{DT}) where DT
    val = exp(a.value)
    out = Variable(val)
    push!(out.parents, (a, val))
    return out
end

function Base.log(a::Variable{DT}) where DT
    out = Variable(log(a.value))
    push!(out.parents, (a, one(DT) / a.value))
    return out
end

function Base.sqrt(a::Variable{DT}) where DT
    val = sqrt(a.value)
    out = Variable(val)
    push!(out.parents, (a, one(DT) / (2 * val)))
    return out
end

function Base.sin(a::Variable{DT}) where DT
    out = Variable(sin(a.value))
    push!(out.parents, (a, cos(a.value)))
    return out
end

function Base.cos(a::Variable{DT}) where DT
    out = Variable(cos(a.value))
    push!(out.parents, (a, -sin(a.value)))
    return out
end

function Base.tanh(a::Variable{DT}) where DT
    val = tanh(a.value)
    out = Variable(val)
    push!(out.parents, (a, one(DT) - val^2))
    return out
end

function Base.abs(a::Variable{DT}) where DT
    out = Variable(abs(a.value))
    push!(out.parents, (a, sign(a.value)))
    return out
end

